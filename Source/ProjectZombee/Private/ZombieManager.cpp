...
// Sums up all bitten
float AZombieManager::conveyor_content(TArray<FConveyorBatch> inputConveyor)
{
    float sum = 0.0;
    for (FConveyorBatch& b : inputConveyor)
        sum += b.amountOfPeople;
    return sum;
}

void AZombieManager::ZombieModel(int& bittenToday, bool& finished)
{
    ...
    Bitten = conveyor_content(conveyor); // current conveyor content (people)
    ...
    float current_content = conveyor_content(conveyor);
    ...
    Bitten = conveyor_content(conveyor);
    ...
    UE_LOG(LogTemp, Log, TEXT("Susceptible: %f, Bitten: %f, Zombies: %f, On day: %d"), Susceptible, Bitten, Zombies, day);
...
void AZombieManager::ZombieModelWithInputs(float tempSusceptible, float tempBitten, float tempZombies, int& bittenToday, bool& finished)
{
    ...
    // ROUND as in Stella (nearest int, half away from zero) -> std::round
    float total_bitten_per_day =
        round(tempZombies * number_of_bites_per_zombie_per_day); // People/day - FMath::Floor 

    // Avoid division by zero by MAX(non_zombie_population, 1)
    float denom = std::max(non_zombie_population, 1.f);
    float number_of_bites_from_total_zombies_on_susceptible =
        round((tempSusceptible / denom) * total_bitten_per_day); // People/day
    ...
    float getting_bitten = number_of_bites_from_total_zombies_on_susceptible;
    // Truncate to not exceed available Susceptible this step (Euler non-negative safeguard)
    getting_bitten = FMath::Min(getting_bitten, floor(tempSusceptible)); // ensure we don't drive below 0
    bittenToday = getting_bitten;

    // --- Conveyor mechanics for Bitten ---
    // 1) Progress existing batches and compute raw outflow (people exiting conveyor)
    for (FConveyorBatch& b : conveyor2)
    {
        b.remainingDays -= DT;  // update remainingDays
        // std::cout << "ConveyorBatch amount of people:" << b.amountOfPeople << " remaining days: "
                  // << b.remainingDays << "\n";
    ...
    TArray<FConveyorBatch> next_conveyor;
    float raw_outflow_people{ 0.f };
    next_conveyor.Reserve(conveyor2.Num()); // Allocates memory??
    for (FConveyorBatch& b : conveyor2)
    {
        if (b.remainingDays <= 0.0)
            raw_outflow_people += b.amountOfPeople;
    ...
    conveyor2 = next_conveyor; // TODO: There may be problems with this

    // 2) Capacity check for new inflow (people)
    float current_content = conveyor_content(conveyor2);
    float free_cap = std::max(0.f, Bitten_capacity - current_content);
    float inflow_people = std::max(0.f, std::min(getting_bitten, free_cap));

    if (inflow_people > 0.f)
        conveyor2.Add(FConveyorBatch{ inflow_people, days_to_become_infected_from_bite });
